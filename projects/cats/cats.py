...
def about(subject):
    ...
    def check_paragraph(paragraph):
        para_words=split(remove_punctuation(lower(paragraph)))  
        for words in subject:
            for wl_words in para_words:
                if words==wl_words:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    count=0
    i=0
    for words in typed_words:
        if i<len(source_words):
            if words==source_words[i]:
                count+=1
        else :
            break
        i+=1
    return count/len(typed_words)*100 if len(typed_words)>0 else (0.0 if len(source_words)>0 else 100.0)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    Arguments:
        ...
        elapsed: an amount of time in seconds1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for words in word_list :
        if words==typed_word:
            return typed_word
    result=min(word_list,key=lambda x:diff_function(typed_word,x,limit))
    return result if diff_function(typed_word,result,limit)<=limit else typed_word 
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if limit==0:
        ...
        if typed[0]==source[0]:
            ...
            return max(len(typed),len(source),key=abs)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[player[i]-player[i-1] for i in range(1,len(player))]for player in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        fastest_player=min(player_indices,key=lambda x:times[x][i])
