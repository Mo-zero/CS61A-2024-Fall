...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if ContainerAnt.can_contain(self,origin_ant):
                self.store_ant(origin_ant)
                place.ant=self
            elif ContainerAnt.can_contain(origin_ant,self):
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained:
            if not self.ant_contained.double_flag:
                self.ant_contained.damage*=2
                self.ant_contained.double_flag=True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not next_place.is_hive:
            if num>=self.lower_bound and num<=self.upper_bound:
                if next_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in list(bees):
            Insect.reduce_health(bee,total_amount)
        # END Problem 5
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.is_container and self.ant_contained==None and not other.is_container:
            return True
        else:
            return False
        # END Problem 8a
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance=0
        while not next_place.is_hive:
            if next_place.ant and next_place.ant is not self:
                result[next_place.ant]=distance
                if next_place.ant.is_container and next_place.ant.ant_contained and next_place.ant.ant_contained is not self:
                    result[next_place.ant.ant_contained]=distance
            if next_place.bees:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        final_damage=(2-0.25*distance-0.0625*self.insects_shot)
        if final_damage<=0:
            final_damage=0
        return final_damage
        # END Problem EC 4
