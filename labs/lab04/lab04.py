def divide(quotients, divisors):
    ...
    """
    return {key: [values for values in divisors if values%key==0] for key in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_b) if distance(city_b,city_c)<=distance(city_c,city_a) else get_name(city_a)
