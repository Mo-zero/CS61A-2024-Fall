...
def floor_div(args):
    ...
    divisior=args.rest
    result=dividend
    while divisior is not nil:
        result=result//divisior.first
        divisior=divisior.rest
    return result
...
def eval_and(expressions):
    ...
    if expressions is nil:
        ...
        elem=calc_eval(expressions.first) if isinstance(expressions.first,Pair) else expressions.first
        if elem is scheme_f:
            return False
